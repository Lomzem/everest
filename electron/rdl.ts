...
export interface ParsedRdlFile {
...
export function parseRdlFile(filePath: string): Promise<ParsedRdlFile> {
	return new Promise((resolve, reject) => {
		const child = spawn(
			'uv',
			['run', '--with', 'systemrdl-compiler', 'python', '-c', parserSource, filePath],
			{
		...
		child.on('close', async (code) => {
			if (code !== 0) {
				reject(new Error(stderr.trim() || `SystemRDL parser exited with code ${code}`));
				return;
			}
			...
			try {
				resolve({
					...
					source: {
						...
						readOnly: true,
						readOnlyReason:
							'This file was opened losslessly. Source-safe edit ranges are not available yet, so the parsed view is read-only.',
					},
				});
			} catch (error) {
				reject(error);
