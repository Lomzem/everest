import { deriveIdentifier } from './identifiers';
export { deriveIdentifier } from './identifiers';
...
export function applyEdit(compilation: Compilation, command: EditCommand): string {
	...
	switch (command.type) {
		case 'set-register-access': {
			if (node!.kind !== 'reg')
				throw new EditError({ message: 'Select a register to set its default access.' });
			const additions: string[] = [];
			const tokens = lex(source.slice(node!.bodyRange.start, node!.bodyRange.end));
			for (const key of ['sw', 'hw'] as const) {
				const value = command[key];
				if (value === undefined) continue;
				if (!['na', 'rw', 'wr', 'r', 'w', 'rw1', 'w1'].includes(value))
					throw new EditError({ message: 'Select a valid access mode.' });
				let depth = 0,
					found = false;
				for (let i = 0; i < tokens.length; i++) {
					const token = tokens[i];
					...
					if (
						...
						const end = tokens.findIndex((t, j) => j > i + 2 && t.text === ';');
						if (end < 0)
							throw new EditError({ message: 'The default access declaration is incomplete.' });
						edits.push({
							start: node!.bodyRange.start + tokens[i + 3].start,
							end: node!.bodyRange.start + tokens[end - 1].end,
							text: value
						});
						found = true;
						break;
				...
				if (!found) additions.push(`default ${key} = ${value};`);
			}
			if (additions.length) edits.push(bodyInsertion(source, node!, additions.join('\n')));
			break;
		}
		case 'add-register': {
			identifier(command.name);
			if (!['addrmap', 'regfile'].includes(node!.kind))
				throw new EditError({ message: 'Select an address map or register file.' });
			if (
				!Number.isInteger(command.width) ||
				command.width < 8 ||
				command.width > 65536 ||
				(command.width & (command.width - 1)) !== 0
			)
				throw new EditError({
					message: 'Register width must be a power of two from 8 through 65,536 bits.'
				});
			if (node!.children.some((child) => child.name === command.name))
				throw new EditError({ message: 'This component name is already in use.' });
			let current = compilation;
			if (command.groupPath && !current.properties.some((p) => p.name === 'doc_group'))
				current = compile(
					applyEdit(current, {
						type: 'upsert-property-definition',
						definition: { name: 'doc_group', type: 'string', components: ['reg'] }
					})
				);
			const parent = current.nodes.find((n) => n.id === command.parentId)!;
			const body = [
				`regwidth = ${command.width};`,
				...(command.title ? [`name = ${JSON.stringify(command.title)};`] : []),
				...(command.groupPath ? [`doc_group = ${JSON.stringify(command.groupPath)};`] : []),
				'default sw = rw;',
				'default hw = rw;'
			];
			const snippet = `reg {\n${body.map((line) => '    ' + line).join('\n')}\n} ${command.name} @ ${command.address};`;
			const result = patches(current.source, [bodyInsertion(current.source, parent, snippet)]);
			const checked = compile(result);
			const error = checked.diagnostics.find((d) => d.severity === 'error');
			if (error) throw new EditError({ message: error.message });
			return result;
		}
		case 'move-to-group': {
			if (node!.kind !== 'reg')
				throw new EditError({ message: 'Select a register to move into a folder.' });
			if (
				command.path.split('/').some((part) => part.trim() !== part) ||
				command.path.includes('//')
			)
				throw new EditError({ message: 'Use folder names separated by a single slash.' });
			if (!command.path && !node!.properties.doc_group) return source;
			let current = compilation;
			if (!current.properties.some((p) => p.name === 'doc_group'))
				current = compile(
					applyEdit(current, {
						type: 'upsert-property-definition',
						definition: { name: 'doc_group', type: 'string', components: ['reg'] }
			...
			return applyEdit(current, {
				type: 'set-property',
				nodeId: command.nodeId,
				property: 'doc_group',
				value: JSON.stringify(command.path)
			});
		}
		case 'rename-group':
		case 'move-group': {
			if (!command.path)
				throw new EditError({ message: 'The root folder cannot be moved or renamed.' });
			const parts = command.path.split('/'),
				label = parts.at(-1)!;
			const name = command.type === 'rename-group' ? command.name : label;
			if (!name.trim() || name.includes('/') || name !== name.trim())
				throw new EditError({
					message: 'Enter a folder name without slashes or surrounding spaces.'
				});
			const parent =
				command.type === 'move-group' ? command.parentPath : parts.slice(0, -1).join('/');
			if (parent === command.path || parent.startsWith(command.path + '/'))
				throw new EditError({ message: 'A folder cannot be moved inside itself.' });
			const nextPath = parent ? `${parent}/${name}` : name;
			if (nextPath === command.path) return source;
			if (compilation.groups?.some((g) => g.path === nextPath))
				throw new EditError({ message: 'A folder with this name already exists here.' });
			let current = compilation;
			for (const reg of compilation.nodes.filter(
				(n) =>
					n.kind === 'reg' &&
					(n.groupPath === command.path || n.groupPath?.startsWith(command.path + '/'))
			))
				current = compile(
					applyEdit(current, {
						type: 'move-to-group',
						nodeId: reg.id,
						path: nextPath + reg.groupPath!.slice(command.path.length)
					})
				);
			return current.source;
		}
		case 'delete-group': {
			if (!command.path) throw new EditError({ message: 'The root folder cannot be deleted.' });
			// One source transaction permits internal references between removed registers.
			const selected = compilation.nodes.filter(
				(n) =>
					n.kind === 'reg' &&
					(n.groupPath === command.path || n.groupPath?.startsWith(command.path + '/'))
			);
			for (const selectedNode of selected) {
				if (!selectedNode.editable)
					throw new EditError({
						message: 'This folder contains generated registers and cannot be deleted.'
					});
				if (
					compilation.nodes.some(
						(n) =>
							...
							n.range.start === selectedNode.range.start &&
							!selected.some((target) => target.id === n.id)
					)
				)
					throw new EditError({
						message: 'A register shares a declaration with an instance outside this folder.'
					});
			}
			const ranges = new Map(selected.map((n) => [n.range.start, n.range]));
			for (const range of ranges.values()) edits.push(removeStatement(source, range));
			break;
		}
		case 'bind-enum': {
			if (node!.kind !== 'field')
				throw new EditError({ message: 'Select a field to edit its enumeration.' });
			let current = compilation;
			const existing = current.enums.find((e) =>
				command.previousStart !== undefined
					? e.range.start === command.previousStart
					: e.name === (command.previousName ?? String(node!.properties.encode?.value ?? ''))
			);
			const reset = node!.properties.reset;
			const selectedIndex = existing?.members.findIndex((m) => m.value === reset?.value) ?? -1;
			const selectedMember = selectedIndex >= 0 ? existing!.members[selectedIndex] : undefined;
			const nextSelected = selectedMember
				? (command.definition.members.find((m) => m.name === selectedMember.name) ??
					(existing?.members.length === command.definition.members.length
						? command.definition.members[selectedIndex]
						: undefined))
				: undefined;
			if (
				reset &&
				!reset.unassigned &&
				reset.text.includes('::') &&
				(command.definition.name !== existing?.name ||
					!command.definition.members.some(
						(m) => reset.text.trim() === `${command.definition.name}::${m.name}`
					))
			)
				current = compile(
					applyEdit(current, {
			...
			if (!command.definition.members.length) {
				if (node!.properties.encode)
					current = compile(
						applyEdit(current, { type: 'remove-property', nodeId: node!.id, property: 'encode' })
					);
				return current.source;
			}
			// Find the declaration again after a reset patch changes source offsets.
			const prior = existing
				? current.enums.find(
						(e) => e.name === existing.name && e.scopeRange?.start === existing.scopeRange?.start
					)
				: undefined;
			current = compile(
				applyEdit(current, {
					type: 'upsert-enum',
					previousStart: prior?.range.start,
			...
			current = compile(
				applyEdit(current, {
					...
					nodeId: node!.id,
					property: 'encode',
					value: command.definition.name
				})
			);
			let resetMember = command.resetMember;
			if (
				resetMember === undefined &&
				nextSelected &&
				reset &&
				(reset.text.includes('::') || evaluate(nextSelected.value) !== reset.value)
			)
				resetMember = nextSelected.name;
			if (resetMember !== undefined)
				return applyEdit(current, {
					type: 'set-property',
					nodeId: node!.id,
					property: 'reset',
					value: `${command.definition.name}::${resetMember}`
				});
			return current.source;
		}
		case 'move-component': {
			const destination = compilation.nodes.find((n) => n.id === command.parentId);
			if (
				!destination ||
				!['addrmap', 'regfile'].includes(destination.kind) ||
				!['reg', 'regfile', 'addrmap', 'mem'].includes(node!.kind) ||
				!node!.parentId
			)
				throw new EditError({
					message: 'Select a valid source component and destination container.'
				});
			if (destination.id === node!.parentId) return source;
			if (destination.id === node!.id || destination.id.startsWith(node!.id + '.'))
				throw new EditError({ message: 'A component cannot be moved inside itself.' });
			if (destination.children.some((n) => n.name === node!.name))
				throw new EditError({ message: 'This component name already exists in the destination.' });
			if (
				compilation.nodes.some(
					(n) =>
						n.id !== node!.id &&
				...
				throw new EditError({
					message: 'This declaration has several instances and cannot be moved separately.'
				});
			const offset = (node!.address ?? 0n) - (destination.address ?? 0n);
			if (offset < 0n)
				throw new EditError({
					message: 'The destination base address is above this component address.'
				});
			let snippet = source.slice(node!.range.start, node!.range.end);
			if (node!.addressRange)
				snippet =
					snippet.slice(0, node!.addressRange.start - node!.range.start) +
					`0x${offset.toString(16)}` +
					snippet.slice(node!.addressRange.end - node!.range.start);
			else {
				const end = snippet.lastIndexOf(';');
				snippet = snippet.slice(0, end) + ` @ 0x${offset.toString(16)}` + snippet.slice(end);
			}
			edits.push(removeStatement(source, node!.range), bodyInsertion(source, destination, snippet));
			break;
		}
		case 'update-title': {
			const current = node!.properties.name;
			const prior = typeof current?.value === 'string' ? current.value : node!.name;
			const derived = deriveIdentifier(command.title);
			if (
				command.deriveIdentifier &&
				node!.name === deriveIdentifier(prior) &&
				derived !== node!.name
			) {
				identifier(derived);
				edits.push({ ...node!.nameRange, text: derived });
			}
			if (current && !current.inherited && !current.editable)
				throw new EditError({ message: 'This display name is generated and cannot be edited.' });
			if (current?.range && !current.inherited)
				edits.push({ ...current.range, text: JSON.stringify(command.title) });
			else edits.push(bodyInsertion(source, node!, `name = ${JSON.stringify(command.title)};`));
			break;
		}
		case 'rename': {
			identifier(command.name);
			...
			if (old) {
				...
				const references = tokens
					...
					.filter(
						({ token, index }) =>
							token.text === old.name &&
							(tokens[index + 1]?.text === '::' ||
								(tokens[index - 1]?.text === '=' && tokens[index - 2]?.text === 'encode')) &&
							compilation.enums
								.filter(
									(e) =>
										e.name === old.name &&
										(!e.scopeRange ||
											(e.scopeRange.start <= token.start && token.start < e.scopeRange.end))
								)
								.sort(
									(a, b) =>
								...
								)[0]?.range.start === old.range.start
					);
				if (old.name !== d.name)
					for (const { token } of references) edits.push({ ...token, text: d.name });
				if (old.name !== d.name) edits.push({ ...old.nameRange, text: d.name });
				...
				for (let i = 0; i < d.members.length; i++) {
					...
					if (member.name !== input.name) {
						edits.push({ ...member.nameRange, text: input.name });
						for (const { index } of references)
							if (tokens[index + 1]?.text === '::' && tokens[index + 2]?.text === member.name)
								edits.push({ ...tokens[index + 2], text: input.name });
					}
					let unchanged = false;
					try {
